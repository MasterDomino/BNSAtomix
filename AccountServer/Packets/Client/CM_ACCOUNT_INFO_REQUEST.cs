...
{
    public class CM_ACCOUNT_INFO_REQUEST : Common.Packets.AccountServer.CM_ACCOUNT_INFO_REQUEST
    {
        public override Packet<AccountPacketOpcode> New()
        {
            return new CM_ACCOUNT_INFO_REQUEST();
        }

        public override void OnProcess(Session<AccountPacketOpcode> client)
        {
            ((AccountSession)client).OnAccountInfoRequest(this);
        }
    }
}
