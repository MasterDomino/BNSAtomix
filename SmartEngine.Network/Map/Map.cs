...
namespace SmartEngine.Network.Map
{
    ...
    public abstract class Map<T>
    {
        ...
        private const ulong ACTORID_NPC_BORDER = 0x4000000000000;
        private const ulong ACTORID_ITEM_BORDER = 0x90000000000000;
        private const ulong ACTORID_CORPSE_BORDER = 0xB0000000000000;
        private const ulong ACTORID_PC_BORDER = 0x1000000000000;
        ...
        public float[] GetRandomPosAroundActor(Actor actor)
        {
            ...
            ret[0] = Global.Random.Next((int)actor.X - 50, (int)actor.X + 50);
            ret[1] = Global.Random.Next((int)actor.Y - 50, (int)actor.Y + 50);
        ...
        public Actor GetActor(ulong id)
        {
            try
            {
                actorsByID.TryGetValue(id, out Actor result);
                return result;
            }
            catch (Exception)
            {
        ...
        public bool RegisterActor(Actor nActor)
        {
            ...
            if (newID != 0)
            {
                ...
                if (!this.actorsByRegion.TryGetValue(nActor.Region, out ConcurrentDictionary<ulong, Actor> region))
                {
        ...
        public void DeleteActor(Actor dActor,bool deleteOnly=false)
        {
            ...
            this.actorsByID.TryRemove(dActor.ActorID, out Actor obj);

            if (this.actorsByRegion.TryGetValue(dActor.Region, out ConcurrentDictionary<ulong, Actor> region))
            {
                region.TryRemove(dActor.ActorID, out Actor removed);
            }
        ...
        public int GetRegionPlayerCount(int region)
        {
            ...
            if (this.actorsByRegion.TryGetValue(region, out ConcurrentDictionary<ulong, Actor> actors))
            {
                ...
                {
                    if (actor.Value.ActorType == ActorType.PC)
                    {
                        count++;
                    }
                }
        ...
        public bool ACanSeeB(Actor A, Actor B)
        {
            ...
            {
                if (System.Math.Abs(difX) > A.SightRange)
                {
                    return false;
                }

                if (System.Math.Abs(difY) > A.SightRange)
                {
                    return false;
                }
        ...
        public bool ACanSeeB(Actor A, Actor B, int sightrange)
        {
            ...
            if (difX > int.MinValue && difY > int.MinValue)
            {
                if (System.Math.Abs(difX) > sightrange)
                {
                    return false;
                }

                if (System.Math.Abs(difY) > sightrange)
                {
                    return false;
                }
        ...
        public void SendVisibleActorsToActor(Actor jActor)
        {
            ...
            for (short deltaY = -1; deltaY <= 1; deltaY++)
            {
                for (short deltaX = -1; deltaX <= 1; deltaX++)
                {
                    int region = jActor.Region + (deltaX * 10000) + deltaY;
                    if (!this.actorsByRegion.TryGetValue(region, out ConcurrentDictionary<ulong, Actor> actors))
                    {
                        continue;
                    }
        ...
        public void TeleportActor(Actor sActor, int x, int y, int z)
        {
            ...
            this.actorsByRegion[sActor.Region].TryRemove(sActor.ActorID, out Actor removed);
            ...
            if (!this.actorsByRegion.TryGetValue(sActor.Region, out ConcurrentDictionary<ulong, Actor> region))
            {
                region = new ConcurrentDictionary<ulong, Actor>();
        ...
        public void SendEventToAllActorsWhoCanSeeActor(T etype, MapEventArgs args, Actor sActor, bool sendToSourceActor)
        {
            try
            {
                for (short deltaY = -1; deltaY <= 1; deltaY++)
                {
                    for (short deltaX = -1; deltaX <= 1; deltaX++)
                    {
                        int region = sActor.Region + (deltaX * 10000) + deltaY;
                        if (!this.actorsByRegion.TryGetValue(region, out ConcurrentDictionary<ulong, Actor> actors))
                        {
                            continue;
                        }
        ...
        public List<Actor> GetActorsAroundArea(int x, int y,int z, int range)
        {
            ...
            try
            {
                for (short deltaY = -1; deltaY <= 1; deltaY++)
                {
                    for (short deltaX = -1; deltaX <= 1; deltaX++)
                    {
                        int region = GetRegion(x, y) + (deltaX * 10000) + deltaY;
                        if (!this.actorsByRegion.TryGetValue(region, out ConcurrentDictionary<ulong, Actor> actors))
                        {
                            continue;
                        }
