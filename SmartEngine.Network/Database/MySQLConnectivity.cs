...
namespace SmartEngine.Network.Database
{
    ...
    public abstract class MySQLConnectivity
    {
        private class MySQLCommand
        {
            ...
            private DateTime nextExecuteTime = DateTime.Now;
        ...
        private readonly ConcurrentQueue<MySQLCommand> waitQueue = new ConcurrentQueue<MySQLCommand>();
        ...
        public void Init(string host, int port, string database, string user, string pass)
        {
            ...
            if (db != null)
            {
                ...
                {
                    Logger.Log.Info("SQL Connection error");
                }
            }
            mysqlPool?.Abort();
        ...
        {
            Logger.Log.Info("Waiting for unprocessed sql queries...");
            while (!CanClose)
            {
                Thread.Sleep(100);
            }
            Logger.Log.Info("Shutting down mysql connectivity...");
            shutingdown = true;
            mysqlPool.Abort();
        }

        private DateTime exeStamp = DateTime.Now;
        private DateTime reqStamp = DateTime.Now;
        ...
        private void ProcessMysql()
        {
            while (true)
            {
                try
                {
                    ...
                    {
                        Logger.Log.Info("Lost connection to Mysql Server...");
                    ...
                    while (waitQueue.TryDequeue(out MySQLCommand i))
                    {
                        try
                        {
                            exeCounter++;
                        ...
                        {
                            Logger.Log.Info("Error on query:" + command2String(i.Command));
                            Logger.Log.Info(ex);
                            i.ErrorCount++;
                            ...
                            {
                                Logger.Log.Info("Error too many times, dropping command");
        ...
        public bool SQLExecuteNonQuery(MySqlCommand cmd_)
        {
            ...
            {
                Logger.Log.Info("Error on query:" + command2String(cmd_));
                Logger.Log.Info(ex);
                if (criticalarea)
                {
                    ClientManager.EnterCriticalArea();
        ...
        public bool SQLExecuteQuery(string sqlstr, out DataRowCollection result)
        {
            ...
            {
                Logger.Log.Info("Error on query:" + sqlstr);
                Logger.Log.Info(ex);
                if (criticalarea)
                {
                    ClientManager.EnterCriticalArea();
