    ...
    public class ColorValueRangeEditor : UITypeEditor
    {
        public override object EditValue(ITypeDescriptorContext context, IServiceProvider provider, object value)
        {
            if (provider != null)
            {
                ...
                try
                {
                    if (context.PropertyDescriptor is IWrappedPropertyDescriptor propertyDescriptor)
                    {
                    ...
                    else
                    {
                        if (context.Instance is Array)
                        {
                            ...
                            for (int i = 0; i < instance.Length; i++)
                            {
                                ...
                                if (wrapperOwner is IWrappedCustomTypeDescriptor)
                                {
                                    wrapperOwner = ((IWrappedCustomTypeDescriptor)wrapperOwner).GetWrapperOwner();
                                }
