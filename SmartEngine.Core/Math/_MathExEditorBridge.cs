...
namespace SmartEngine.Core.Math
{
    public abstract class _MathExEditorBridge
    {
        ...
        private static Assembly A(string A)
        {
            ...
            if (string.IsNullOrEmpty(extension) || (!string.Equals(extension, ".dll", StringComparison.CurrentCultureIgnoreCase)))
            {
                A += ".dll";
            }
            ...
            {
                Logger.Log.Fatal($"Assembly not found \"{A}\".");
            }
            ...
            {
                Logger.Log.Fatal($"Load assembly failed \"{assemblyName.FullName}\".");
                return null;
            }
            return assembly2;
        }
